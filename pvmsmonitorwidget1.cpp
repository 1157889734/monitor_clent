...
//    mainLayout->setSpacing(0);
    for(int i=0; i <mlist.count(); i++)
    {
        ...
        QHBoxLayout *layout = new QHBoxLayout;
        hLayoutList->append(layout);
         qDebug()<<"*******************add hBoxlayoutt"<<"******i:***"<<"index="<<i<<endl;
        if(layout){
            layout->addWidget(video);
//            QLabel* pTestLabel = new QLabel();
//            QString strTitle = QString("chanel %1").arg(i);
//            pTestLabel->setFont(QFont("宋体",20));
//            layout->addWidget(pTestLabel);
//            layout->setMargin(0);
//            layout->setSpacing(0);
            qDebug()<<"*******************add video addWidget"<<"******i:***"<<"index="<<i<<"video:"<<video<<"hLayoutList:"<<layout<<endl;
        }
        QWidget *widget = new QWidget();
        widget->setLayout(layout);
        mainLayout->addWidget(widget);
        qDebug()<<"*******************mainLayout addWidget"<<"******i:***"<<"index="<<i<<endl;
        m_lstParentWidget.append(widget);
    }
    playwidget = new QWidget(this);
    playwidget->setGeometry(0, 0, 782, 630);

//    QLabel *pTestLabel = new QLabel(video);
//    pTestLabel->setGeometry(0, 360, 130, 50);
//    pTestLabel->setStyleSheet("QLabel{color:rgb(55, 82, 103);font: 24pt;background-color: rgb(0, 0, 0);}");
//    pTestLabel->setAttribute(Qt::WA_TranslucentBackground, true); //设置控件背景透明
//    pTestLabel->show();
//    QLabel *channelLabel = new QLabel(this);
//    channelLabel->setGeometry(0, 400, 100, 50);
//    channelLabel->setStyleSheet("QLabel{color:rgb(55, 82, 103);font: 24pt;background-color: rgb(0, 0, 0);}");
//    channelLabel->setAttribute(Qt::WA_TranslucentBackground, true);
//    channelLabel->show();
...
//    playwidget->installEventFilter(this);     //播放窗体注册进事件过滤器
...
//    playwidget->setLayout(mainLayout);
    ...
    for(int i=0; i<mlist.count(); i++){
        QVideoWidget *pLocalVideo = videoList->value(i);
        QMediaPlayer *pLocalplayer = playerlist->value(i);
        if(pLocalVideo && pLocalplayer){
...
void pvmsMonitorWidget::showMedia(int ch)
{
    ...
    for(int i=0;i<4;i++)
    {
        if(ch!=i)
        {
            m_lstParentWidget.value(i)->hide();
        }
    }
    ...
    if(pLocalVideo && pLocalplayer){
        ...
        pLocalplayer->stop();
         pLocalVideo->hide();
        pLocalVideo->showFullScreen();
         pLocalplayer->setVideoOutput(pLocalVideo);
         pLocalplayer->play();
        QHBoxLayout *pLayout =hLayoutList->value(ch);
        bool bShow = m_lstParentWidget.value(ch)->isVisible();
        if(bShow)
        {
             qDebug()<<"state:"<<pLocalplayer->state()<<"show-media1111**************ich="<<ch<<"video show rect:"<<m_lstParentWidget.value(ch)->rect()\
    ...
    m_lstParentWidget.value(ch)->show();

    for(int i=0;i<4;i++)
    {
        bool bParentShow = m_lstParentWidget.value(i)->isVisible();
        QWidget *video = videoList->value(i);
