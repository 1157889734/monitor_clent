...
int set_sp_plane(struct sp_dev *dev, struct sp_plane *plane, struct sp_crtc *crtc, struct sp_bo *bo, int x, int y, int nw, int nh)
{
	int ret;
    int w, h;
...
//    printf("--x:%d,y:%d, w:%d, h:%d, nw:%d, nh:%d \n", x, y, w, h, nw, nh);
    ret = drmModeSetPlane(dev->fd, plane->plane->plane_id,
            crtc->crtc->crtc_id, bo->fb_id, 0, x, y, nw, nh,
            0, 0, w << 16, h << 16);
