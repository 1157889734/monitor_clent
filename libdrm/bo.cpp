		...
		uint32_t height, uint8_t a, uint8_t r, uint8_t g, uint8_t b)
{
	...
	for (i = y; i < ymax; i++) {
        uint8_t *luma = (uint8_t *)bo->map_addr + i * bo->pitch;
	...
	for (i = y; i < ymax / 2; i++) {
        uint8_t *chroma = (uint8_t *)bo->map_addr + (i + height) * bo->pitch;
...
void draw_rect_rgba(struct sp_bo *bo, uint32_t x, uint32_t y, uint32_t width,
        uint32_t height, uint8_t*data)
{
    uint32_t src_p = width*4;
    uint32_t i, j, xmax = x + width, ymax = y + height;
    if (xmax > bo->width)
        xmax = bo->width;
    if (ymax > bo->height)
    ...
    for (i = 0; i < ymax; i++) {
        ...
        uint8_t *row_s = data + i * src_p;
        for (j = 0; j < xmax; j++)
        {
            uint8_t *pixel_s = row_s + j * 4;
...
static int map_sp_bo(struct sp_bo *bo)
{
    ...
    if(bo->format == DRM_FORMAT_NV12)
    {
        memset(bo->map_addr, 0x0, bo->pitch*bo->height);
        memset(bo->map_addr+bo->pitch*bo->height, 0x80, bo->size-bo->pitch*bo->height);
    }
	return 0;
}
